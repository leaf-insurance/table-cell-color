...
function k(h, e) {
  const t = h.getBoundingClientRect(), { width: o, height: i, x: r, y: l } = t, { clientX: d, clientY: n } = e;
  return {
    width: o,
    height: i,
    x: d - r,
...
function g(h, e) {
  return e.parentNode.insertBefore(h, e);
}
function m(h, e = !0) {
  const t = document.createRange(), o = window.getSelection();
  t.selectNodeContents(h), t.collapse(e), o.removeAllRanges(), o.addRange(t);
}
class a {
  /**
...
class $ {
   ...
   */
  constructor(e, t, o, i) {
    this.readOnly = e, this.api = t, this.data = o, this.config = i, this.wrapper = null, this.table = null, this.cellColors = [
    ...
    ], this.toolboxColumn = this.createColumnToolbox(), this.toolboxRow = this.createRowToolbox(), this.toolboxCell = this.createCellToolbox(), this.createTableWrapper(), this.hoveredRow = 0, this.hoveredColumn = 0, this.selectedRow = 0, this.selectedColumn = 0, this.selectedCell = { row: 0, column: 0 }, this.tunes = {
      withHeadings: !1
    }, this.resize(), this.fill(), this.focusedCell = {
      row: 0,
      column: 0
    }, this.documentClicked = (r) => {
      const l = r.target.closest(`.${s.table}`) !== null, d = r.target.closest(`.${s.wrapper}`) === null;
      (l || d) && this.hideToolboxes();
   ...
   */
  bindEvents() {
    document.addEventListener("click", this.documentClicked), this.table.addEventListener("mousemove", E(150, (e) => this.onMouseMoveInTable(e)), { passive: !0 }), this.table.onkeypress = (e) => this.onKeyPressListener(e), this.table.addEventListener("keydown", (e) => this.onKeyDownListener(e)), this.table.addEventListener("focusin", (e) => this.focusInTableListener(e)), this.table.addEventListener("click", (e) => this.cellClickListener(e));
  }
  /**
  ...
  createRowToolbox() {
    return new w({
      ...
      onOpen: () => {
        this.selectRow(this.hoveredRow), this.hideColumnToolbox(), this.hideCellToolbox();
      },
      onClose: () => {
        this.unselectRow();
      }
   ...
   */
  createCellToolbox() {
    const e = this.cellColors.map((t) => ({
      label: this.api.i18n.t(t.name),
      icon: `<div style="width: 100%; height: 100%; border-radius: 3px; background-color: ${t.value}; border: 1px solid #ccc;"></div>`,
      onClick: () => {
        this.setCellColor(this.selectedCell.row, this.selectedCell.column, t.value), this.hideToolboxes();
      }
    }));
    return new w({
      api: this.api,
  ...
  setCellColor(e, t, o) {
    ...
    i && (i.style.backgroundColor = o, i.dataset.backgroundColor = o);
  }
  /**
   ...
   */
  setCellContent(e, t, o, i = {}) {
    const r = this.getCell(e, t);
    r.innerHTML = o, i.backgroundColor && (r.style.backgroundColor = i.backgroundColor, r.dataset.backgroundColor = i.backgroundColor);
  }
  /**
  ...
  addColumn(e = -1, t = !1) {
    ...
    if (this.config && this.config.maxcols && this.numberOfColumns >= this.config.maxcols)
      return;
    for (let l = 1; l <= this.numberOfRows; l++) {
      let d;
      const n = this.createCell();
      if (e > 0 && e <= o ? (d = this.getCell(l, e), g(n, d)) : d = this.getRow(l).appendChild(n), l === 1) {
        const u = this.getCell(l, e > 0 ? e : o + 1);
        u && t && m(u);
      }
    }
  ...
  addRow(e = -1, t = !1) {
    ...
    if (this.config && this.config.maxrows && this.numberOfRows >= this.config.maxrows && d)
      return;
    ...
    const l = this.getRowFirstCell(o);
    l && t && m(l);
    const d = this.wrapper.querySelector(`.${s.addRow}`);
    return this.config && this.config.maxrows && this.numberOfRows >= this.config.maxrows && d && d.classList.add(s.addRowDisabled), o;
  }
  /**
  ...
  deleteColumn(e) {
    ...
    const t = this.wrapper.querySelector(`.${s.addColumn}`);
    t && t.classList.remove(s.addColumnDisabled);
  }
  /**
  ...
  deleteRow(e) {
    ...
    const t = this.wrapper.querySelector(`.${s.addRow}`);
    t && t.classList.remove(s.addRowDisabled), this.addHeadingAttrToFirstRow();
  }
  /**
   ...
   */
  createTableWrapper() {
    if (this.wrapper = c("div", s.wrapper), this.table = c("div", s.table), this.readOnly && this.wrapper.classList.add(s.wrapperReadOnly), this.wrapper.appendChild(this.toolboxRow.element), this.wrapper.appendChild(this.toolboxColumn.element), this.wrapper.appendChild(this.toolboxCell.element), this.wrapper.appendChild(this.table), !this.readOnly) {
      const e = c("div", s.addColumn, {
        innerHTML: v
      }), t = c("div", s.addRow, {
        innerHTML: v
      });
   ...
   */
  computeInitialSize() {
    const e = this.data && this.data.content, t = Array.isArray(e), o = t ? e.length : !1, i = t ? e.length : void 0, r = o ? e[0].length : void 0, l = Number.parseInt(this.config && this.config.rows), d = Number.parseInt(this.config && this.config.cols), n = !isNaN(l) && l > 0 ? l : void 0, u = !isNaN(d) && d > 0 ? d : void 0;
    return {
      rows: i || n || 2,
      cols: r || u || 2
    };
    ...
    if (e && e.content)
      for (let t = 0; t < e.content.length; t++)
        for (let o = 0; o < e.content[t].length; o++) {
          const i = e.content[t][o], r = typeof i == "object" ? i.content : i, l = typeof i == "object" ? { backgroundColor: i.backgroundColor } : {};
          this.setCellContent(t + 1, o + 1, r, l);
        }
  }
  /**
   ...
   */
  cellClickListener(e) {
    const t = e.target.closest(`.${s.cell}`);
    if (!t)
      return;
    this.unselectCell();
    const o = this.getRowByCell(t), i = Array.from(this.table.querySelectorAll(`.${s.row}`)).indexOf(o) + 1, r = Array.from(o.querySelectorAll(`.${s.cell}`)).indexOf(t) + 1;
    this.selectCell(i, r), this.updateToolboxesPosition();
  }
  /**
  ...
  selectCell(e, t) {
    ...
    o && (this.selectedCell = {
      row: e,
      column: t
    }, o.style.backgroundColor && !o.dataset.backgroundColor && (o.dataset.backgroundColor = o.style.backgroundColor), o.classList.add(s.cellActive));
  }
  /**
  ...
  unselectCell() {
    if (this.selectedCell.row <= 0 || this.selectedCell.column <= 0)
      return;
    const e = this.getCell(this.selectedCell.row, this.selectedCell.column);
    e && (e.classList.remove(s.cellActive), e.dataset.backgroundColor && (e.style.backgroundColor = e.dataset.backgroundColor)), this.selectedCell = {
      row: 0,
      column: 0
    };
  }
  ...
  updateToolboxesPosition(e = this.hoveredRow, t = this.hoveredColumn) {
    if (this.isColumnMenuShowing || t > 0 && t <= this.numberOfColumns && this.toolboxColumn.show(() => ({
      ...
      const o = this.getRow(e), { fromTopBorder: i } = C(this.table, o), { height: r } = o.getBoundingClientRect();
      return {
        top: `${Math.ceil(i + r / 2)}px`
      };
    }), this.selectedCell.row > 0 && this.selectedCell.column > 0) {
      const o = this.getCell(this.selectedCell.row, this.selectedCell.column);
      if (o) {
        const { fromTopBorder: i, fromLeftBorder: r } = C(this.table, o), { width: l, height: d } = o.getBoundingClientRect();
        this.toolboxCell.show(() => ({
          top: `${Math.ceil(i)}px`,
          left: `${Math.ceil(r + l)}px`
        }));
      }
    }
  }
  ...
  getHoveredCell(e) {
    ...
    const { width: i, height: r, x: l, y: d } = k(this.table, e);
    return l >= 0 && (o = this.binSearch(
      this.numberOfColumns,
      (n) => this.getCell(1, n),
      ({ fromLeftBorder: n }) => l < n,
      ({ fromRightBorder: n }) => l > i - n
    )), d >= 0 && (t = this.binSearch(
      this.numberOfRows,
      (n) => this.getCell(n, 1),
      ({ fromTopBorder: n }) => d < n,
      ({ fromBottomBorder: n }) => d > r - n
    )), {
  ...
  getData() {
    const e = [];
    for (let t = 1; t <= this.numberOfRows; t++) {
      const o = this.table.querySelector(`.${s.row}:nth-child(${t})`), i = Array.from(o.querySelectorAll(`.${s.cell}`));
      i.every((l) => !l.textContent.trim()) || e.push(i.map((l) => {
        const d = l.dataset.backgroundColor || l.style.backgroundColor;
...
class D {
   ...
   */
  render() {
    return this.table = new $(this.readOnly, this.api, this.data, this.config), this.container = c("div", this.api.styles.block), this.container.appendChild(this.table.getWrapper()), this.table.setHeadingsSetting(this.data.withHeadings), this.container;
  }
  /**
  ...
  renderSettings() {
    return [
      ...
      {
        label: this.api.i18n.t("Copy"),
        icon: R,
        isActive: !1,
        closeOnActivate: !0,
        toggle: !1,
        onActivate: async () => {
          await navigator.clipboard.writeText(["asdf"]);
        }
      }
    ];
  }
   ...
   */
  onPaste(e) {
    const t = e.detail.data, o = t.querySelector(":scope > thead, tr:first-of-type th"), r = Array.from(t.querySelectorAll("tr")).map((l) => Array.from(l.querySelectorAll("th, td")).map((n) => n.innerHTML));
    this.data = {
      withHeadings: o !== null,
      content: r
